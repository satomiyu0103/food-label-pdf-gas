...
4. スプレッドシートを開き直し、メニュー **商品DB → インプット PDF 一括処理** を実行
5. alert のサマリで成功・失敗件数を確認。失敗 PDF はエラーフォルダへ隔離される
...
| `src/spreadsheet.js` | JSON → 商品DB 行追加・drive_file_name 更新 | 実装済（重複チェックは Phase 2） |
