...
3. インプットフォルダに PDF を配置（**1 PDF 1 商品**。複数商品を 1 ファイルにまとめない）
...
- **1 PDF = 1 商品** — 商品ごとに PDF を分ける。複数商品を 1 ファイルにまとめない
- 1 商品の表面・裏面・複数ページは同一 PDF でよい
- 運用違反 PDF が混入した場合、Gemini は主対象 1 件のみ抽出し `confidence_notes` に記録する（検知用。正しい運用の代替ではない）

詳細: [doc/specs/02_要件定義.md](../doc/specs/02_要件定義.md) の「運用ルール（入力）」
