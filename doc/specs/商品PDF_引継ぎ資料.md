...
想定する入力は、商品ラベル、商品仕様書、食品表示、栄養成分表示を含む PDF。**1 PDF = 1 商品 = 1 シート行** を運用ルールで徹底する。複数商品を 1 PDF にまとめない。コードで複数行へ展開する機能は見送りとする（2026-07-01 確定）。
...
| 項目 | 理由 |
|---|---|
| 1 PDF 複数商品の複数行展開 | 運用で 1 PDF 1 商品を徹底。データ形・他ツール連携のため |
...
- 複数商品が見える場合（運用違反）は主対象の商品 1 件のみを返し、`confidence_notes` に複数商品の可能性を書く（検知用の安全網）
...
| 複数ページ | 同一商品の後続ページに表示情報がある場合に拾えるか確認する |
| 運用違反（複数商品 PDF） | 投入しないのが正。万一混入時は主対象 1 件のみ・`confidence_notes` に記録されるか確認する |
...
| 項目 | 初期案 | 決めるタイミング |
|---|---|---|
| 1 PDF に複数商品 | **運用で 1 PDF 1 商品**（コード展開は見送り） | 2026-07-01 確定 |
